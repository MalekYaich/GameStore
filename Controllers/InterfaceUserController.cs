...
namespace projetDotNet.Controllers
{
    public class InterfaceUserController : Controller
    {
        ...
        {
            var n = (from m in db.cart
                     select m.numeroCommande).Distinct().Count();
            foreach (var item in db.cart)
            {
                if (item.numeroCommande == null && item.clientID == int.Parse(System.Web.HttpContext.Current.User.Identity.Name))
                    item.numeroCommande = n + 1;
            }
            db.SaveChanges();
